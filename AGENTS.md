...
To test in a browser without a Strava login, run `pnpm dev:mock` instead of `pnpm dev`: it migrates and uses the local database (`apps/web/.data/local.db`) and enables **Dev: sign in as a test athlete** on the landing page (`/api/dev/login`), whose Strava data is generated by `apps/web/src/lib/strava-mock.ts`. The mock refuses to run against a remote database — plain `pnpm dev` points at the live Turso DB, so never test writes there.

**Migrations ship with the deploy.** `.github/workflows/deploy.yml` runs `pnpm --filter @trihards/db migrate` against production (repo secrets `TURSO_DATABASE_URL` / `TURSO_AUTH_TOKEN`) *before* it pushes the new image, and the old code keeps serving while it runs. So keep migrations additive (new nullable columns, new tables); dropping or renaming a column needs two deploys: stop using it first, remove it after.
