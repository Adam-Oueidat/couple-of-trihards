...
import { matchSessions, daysUntilRace, SEED_PLAN, type StravaActivity } from "../src";
...
describe("matchSessions today handling", () => {
  ...
  it("flags a session in the past as 'missed' and a later one as 'upcoming'", () => {
    const [a, b] = SEED_PLAN.sessions;
    // Pretend "today" is the first session's day: it is today, the next is upcoming.
    const result = matchSessions(SEED_PLAN, [], undefined, a.date);
    ...
    const later = matchSessions(SEED_PLAN, [], undefined, b.date);
  ...
  it("does not flag a completed Sunday session as missed when local 'today' is Monday", () => {
    ...
    const result = matchSessions(SEED_PLAN, [act], undefined, today);
...
describe("daysUntilRace", () => {
  it("counts whole days from the given local date to the race", () => {
    const race = new Date(SEED_PLAN.raceDate + "T12:00:00");
    ...
    expect(daysUntilRace(SEED_PLAN, today)).toBe(10);
  });

  it("never goes negative once the race has passed", () => {
    expect(daysUntilRace(SEED_PLAN, "2099-01-01")).toBe(0);
  });
});
